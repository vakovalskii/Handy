...
fn copy_bundled_bindings(out: &std::path::Path) {
    let bindings =
        std::fs::read_to_string("src/bindings.rs").expect("Failed to read bundled bindings.rs");
    let bindings = prepare_bundled_bindings(&bindings);

    std::fs::write(out.join("bindings.rs"), bindings).expect("Failed to write bindings.rs");
}

fn prepare_bundled_bindings(bindings: &str) -> String {
    let bindings = strip_bindgen_const_layout_tests(bindings);
...
fn strip_bindgen_const_layout_tests(bindings: &str) -> String {
    const PREFIX: &str =
